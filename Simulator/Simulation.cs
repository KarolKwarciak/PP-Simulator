...
public class Simulation
{
        ...
        List<Point> positions, string moves)
    {
        ...
        {
            throw new ArgumentException("The number of positions does not match the number of creatures.");
        }
        ...
        {
            throw new ArgumentException("Moves must include a value and cannot be empty or null.");
        }
        Map = map ?? throw new ArgumentNullException(nameof(map));

        Moves = Validate(moves);
    ...
    private string Validate(string moves) => new(moves.Where(c => validMoves.Contains(Char.ToLower(c))).ToArray());
    ...
    public void Turn()
    {
        if (Finished == true)
        {
            throw new InvalidOperationException("The simulation has already been completed!");
        }
